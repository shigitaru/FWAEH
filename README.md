...
Пустые **замеры** (`measurements_json`) для товаров в PostgreSQL приложение добивает **самое при старте воркера** (миграция схемы) и затем дошивает **по входящим запросам** (раз в минуту, если есть товары без замеров). Отдельно ничего запускать не нужно.
